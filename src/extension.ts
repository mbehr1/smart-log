...
export function activate(context: vscode.ExtensionContext) {
  ...
  let smartLogApi = {
    onDidChangeSelectedTime(listener: any) {
      return smartLogs.onDidChangeSelectedTime(listener);
    }, // todo looks wrong... any help on proper param types appreciated
  };
...
export default class SmartLogs implements vscode.TreeDataProvider<EventNode>, vscode.TextDocumentContentProvider, vscode.Disposable {
  ...
  private _defaultTimeRegex = new RegExp(
    '^([0-2][0-9]|[0-2][0-9][0-9][0-9])-([0-1][0-9])-([0-3][0-9]) ([0-2][0-9]):([0-5][0-9]):([0-5][0-9]),([0-9][0-9][0-9])'
  );
  private _timeRegex = vscode.workspace.getConfiguration().get<string>('smart-log.timeRegex')
    ? new RegExp(<string>vscode.workspace.getConfiguration().get<string>('smart-log.timeRegex'))
    : this._defaultTimeRegex;
  ...
  constructor() {
    ...
    this._supportedLanguageIds.forEach((value) => {
      this._subscriptions.push(vscode.languages.registerHoverProvider({ language: value }, this));
    });

    // announce time updates on selection of lines:
    this._subscriptions.push(
      vscode.window.onDidChangeTextEditorSelection(
        util.throttle(async (ev) => {
    ...
    this._subscriptions.push(
      vscode.commands.registerCommand('smart-log.openAsTextDoc', (...args) => {
    ...
    this._subscriptions.push(
      vscode.commands.registerCommand('smart-log.openAsTextDiff', (...args) => {
        const node = <EventNode>args[0];
        if (node) {
          console.log(`smart-log.openAsTextDoc called with node='${node.label}' and uri='${node.uri?.toString()}'`);
          ...
          if (this._smartLogTreeView && this._smartLogTreeView.selection.length > 0) {
            ...
            if (selectedNode !== node) {
              ...
              vscode.commands.executeCommand('vscode.diff', uri1, uri2, `events '${selectedNode.label}'<->'${node.label}'`, {
                preview: false,
              });
              return;
            }
    ...
    this._subscriptions.push(
      vscode.commands.registerTextEditorCommand('smart-log.adjustTime', async (textEditor) => {
    ...
    this._subscriptions.push(
      vscode.commands.registerCommand('smart-log.toggleTimeSync', () => {
    ...
    this._subscriptions.push(
      vscode.commands.registerTextEditorCommand('smart-log.sendTimeSyncEvents', async (textEditor) => {
    ...
    this._subscriptions.push(
      vscode.commands.registerTextEditorCommand('smart-log.sendTime', async (textEditor) => {
    ...
    if (this._documents.has(doc.uri.toString())) {
      ...
      if (this._rewriteLanguageIds.has(doc.languageId)) {
        ...
        if (langSet?.firstLineRegEx.exec(firstLine.text)) {
          console.log(
            `smart-log.addTextDocument changing languageId from ${doc.languageId} to ${langSet.newLanguageId} due to first line match of ${langSet.firstLineRegEx}`
          );
  ...
  public async provideHover(doc: vscode.TextDocument, position: vscode.Position): Promise<vscode.Hover | null> {
    ...
    if (posTime && posTime.valueOf() > 0) {
      ...
      return new vscode.Hover({
        language: 'smart-log',
        value: `calculated time: ${posTime.toLocaleTimeString()}.${posTime.valueOf() % 1000} line#=${position.line}`,
      }); // posCloseTo=${timePos?.line}` });
    }
  ...
  async provideTimeByData(data: DataPerDocument, line: number): Promise<Date> {
    if (data.cachedTimes && line < data.cachedTimes.length) {
      ...
      return vscode.window.withProgress(
        ...
        async (progress, cancelToken): Promise<Date> => {
            ...
            if (name && identifyRegexStr) {
              ...
              if (identifyRegex.exec(doc.getText())) {
                ...
                for (let l = 0; l < nrLines; ++l) {
                  if (this.parseDateForLine(data, l) !== undefined) {
                    linesWithDate++;
                  }
                }
                ...
                if (matchAccurracy > 50) {
                  // good enough, lets keep this
                  break;
                }
              }
            }
    ...
    if (identifiedFileConfig) {
      ...
      if (events) {
        for (let i = 0; i < events.length; ++i) {
          ...
          if (event.regex) {
            // level, label, icon and decorationId are optional
            rEvents.push({
              regex: new RegExp(event.regex),
              label: event.label,
              level: event.level ? event.level : 0,
              decorationId: event.decorationId,
              timeSyncId: event.timeSyncId,
              timeSyncPrio: event.timeSyncPrio,
              icon: event.icon ? new vscode.ThemeIcon(event.icon) : undefined,
            });
          }
        }
      }
      ...
      let eventRoot: EventNode = {
        id: util.createUniqueId(),
        label: `${identifiedFileConfig.name}:${path.basename(doc.uri.fsPath)}`,
        uri: doc.uri,
        parent: null,
        children: [],
      };
        ...
        if (level === 1) {
          ...
          if (parent.children.length === 0) {
            ...
            parent.children.push({
              id: util.createUniqueId(),
              label: `(no parent level ${level - 1} event)`,
              uri: doc.uri,
              parent: parent,
              children: [],
      ...
      return vscode.window.withProgress({ location: vscode.ProgressLocation.Notification }, async (progress) => {
                ...
                if ((match = ev.regex.exec(line.text))) {
                  ...
                  if (ev.level > 0) {
                    ...
                    parentNode.children.push({
                      id: util.createUniqueId(),
                      label: label,
                      uri: doc.uri.with({ fragment: `${line.lineNumber}` }),
                      parent: parentNode,
                      children: [],
                      icon: ev.icon,
                    });
                  ...
                  if (ev.timeSyncId && ev.timeSyncPrio) {
                    ...
                    data.timeSyncs.push([
                      ...
                      {
                        id: ev.timeSyncId,
                        value: match[match.length - 1].toLowerCase(),
                        time: await this.provideTimeByData(data, line.lineNumber),
                        prio: ev.timeSyncPrio,
                      },
                    ]);
                  }
                }
        ...
        decorations.forEach((value, key) => {
          // todo if a prev. DecorationType is missing it's not set!
        ...
        if (doReveal) {
          this._smartLogTreeView?.reveal(data.eventTreeNode, { select: false, focus: false, expand: false });
        }
  ...
  public getChildren(element?: EventNode): EventNode[] | Thenable<EventNode[]> {
    ...
    if (!element) {
      // if no element we have to return the root elements.
      let nodeArray: EventNode[] = [];
      this._documents.forEach((data) => {
        if (data.eventTreeNode) {
          nodeArray.push(data.eventTreeNode);
        }
      });
      return nodeArray;
    } else {
                ...
                if (remoteSyncEv.value === localSyncEv.value) {
                  console.log(
                    `   got id='${remoteSyncEv.id}',prio=${remoteSyncEv.prio} and value='${
                      remoteSyncEv.value
                    } match at ${remoteSyncEv.time.toLocaleTimeString()} with local line ${localSyncLineEv[0]}, prio=${localSyncEv.prio}`
                  );
